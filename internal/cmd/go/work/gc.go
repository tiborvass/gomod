...
import (
	...
	"github.com/tiborvass/gomod/internal/cmd/objabi"
	"github.com/tiborvass/gomod/internal/cmd/sys"
	"crypto/sha1"
)
...
func (gcToolchain) gc(b *Builder, a *Action, archive string, importcfg []byte, symabis string, asmhdr bool, gofiles []string) (ofile string, output []byte, err error) {
	...
	if p.Standard && (p.ImportPath == "github.com/tiborvass/gomod/internal/cpu" || p.ImportPath == "github.com/tiborvass/gomod/internal/bytealg") {
	...
	if p.Standard {
		switch p.ImportPath {
		case "bytes", "github.com/tiborvass/gomod/internal/poll", "net", "os", "runtime/pprof", "runtime/trace", "sync", "syscall", "time":
			extFiles++
		}
	}
	if extFiles == 0 {
