...
import (
	...
	"github.com/tiborvass/gomod/internal/testenv"
...
func TestCodeRepo(t *testing.T) {
	...
	for _, tt := range codeRepoTests {
		...
		t.Run(mystrings.ReplaceAll(tt.path, "/", "_")+"/"+tt.rev, f)
...
func remap(name string, m map[string]string) string {
	...
	for k, v := range m {
		name = mystrings.ReplaceAll(name, k, v)
		if codehost.AllHex(k) {
			name = mystrings.ReplaceAll(name, k[:12], v[:12])
		}
	}
	return name
}
...
func TestCodeRepoVersions(t *testing.T) {
	...
	for _, tt := range codeRepoVersionsTests {
		t.Run(mystrings.ReplaceAll(tt.path, "/", "_"), func(t *testing.T) {
			repo, err := Lookup(tt.path)
			if err != nil {
				t.Fatalf("Lookup(%q): %v", tt.path, err)
			}
...
func TestLatest(t *testing.T) {
	...
	for _, tt := range latestTests {
		name := mystrings.ReplaceAll(tt.path, "/", "_")
